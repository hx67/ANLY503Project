---
title: "What is the impact of words and sentiments in lyrics on People¡¯s attitude towards the songs?"
author: "Group3: Hong Xu, Tianjing Cai, Lufei Wang, Zihao Li, Jiaxu He"
...
```
<center>
<font size="4"> [Graph 2(matplotlib), Source Code](https://github.com/hx67/ANLY503Project/blob/master/Part1/code/Graph2.py)</font>
</center>
...
```
<center>
<font size="4"> [Graph 3(matplotlib), Source Code](https://github.com/hx67/ANLY503Project/blob/master/Part1/code/Graph3.py)</font>
</center>
...
#### To collect Twitter data of singer and song for the past 10 years, we used a Python package called GetOldTweets which enable us to put keywords and date range to collect related tweets. 300 Tweets were collected for each singer and song combination during the week when they appeared on Billboard top 100 songs. Collected Tweets were messy and contain lots of non-alphabetic symbols. And below is the top 20 non-alphabetic symbols in the Tweets.
...
<center>
<font size="4"> [Graph 5(matplotlib), Source Code](https://github.com/hx67/ANLY503Project/blob/master/Part1/code/Graph5.py)</font>
</center>

#### The Hyphen was most frequently used because people are more likely to refer a singer and his/her song together. @ and # are frequently used because they have their actual functionality in Twitter. We could also see that ! has a lot of higher frequency than ?. From my interpretation, it is due to the mood when user sending those Tweets. After listening to the song or hearing related news from some artists, Twitter users are less likely to raise questions, rather Twitter users are more likely to send Tweets to directly convey their emotions (like or dislike) and from sentiment analysis, we could see Tweets with ! has sentiment score average at 0.6. 
...
<center>
<font size="4"> [Graph 6(matplotlib), Source Code](https://github.com/hx67/ANLY503Project/blob/master/Part1/code/Graph6.py)</font>
</center>
...
<center>
<font size="4"> [Graph 7(matplotlib), Source Code](https://github.com/hx67/ANLY503Project/blob/master/Part1/code/Graph7.py)</font>
</center>
...
#### Now, our team wants to study the trend for the development of popular music throughout the years. The plots will show how the song names changed and what kind of songs are likely to stay in the Billboard longer:
...
### (1). How did the Song Names Change from 1990 to 2017?  
...
#### - Shorter Song names makes songs more popular
...
#### - This time we calculated average number of words in song names again, but split by Top 50 and Bottom 50   
...
#### - For the relatively more popular songs, decrease in length of song names is a trend
...
#### - In the 90's, A song with short names is more likely to stay on the list for longer
#### - The Average number of weeks popular songs can stay on billboard have dropped significantly present todays.
...
#### - The word 'like' is the mostly used by the artists
...
#### It seems there is a logarithm relationship between the count and the mean total weeks of songs on the Billboard. And the plots suggest that quantatiy in popular music field is negatively correlated to the quality.

# 4.What is the characters for those popular songs' lyrics?
#### In this sub-question, we want to study what kind of lyrics or what sentiment/mood in lyrics is more likely to make the song popular. In the first part of these project, the team studies a sample singer, Talyor Swift, thoroughly and uses EDA plots to study the pattern of the whole dataset which contains the lyrics for all popular songs in the Billboard Top100. These part will lay a foundation for the study in the second part of the project.

### (1).Talyor Swift Example
#### Last sub-question shows that Talyor Swift is one of the most successful singers in the past ten years. Besides, her songs include many different moods or themes. Talyor Swift is a good sample to start our study. We use the lyrics in all Talyor's songs in this part.

```{r warning=FALSE, ,fig.align='center', include=FALSE, out.width='100%'}
library(magrittr)
library(stringr)
library(dplyr)
library(ggplot2)
...
#### The average word count for the tracks stands close to 375, and the chart shows that the maximum number of songs fall in between 345 to 400 words. The density plot shows that the distribution is close to a normal distribution.
...
#### This plot shows a similar result. This means, to be a superstar, you need to convey more information in your songs, with which you can influence your audience.

#### Now let us start the study on the sentiment in songs. The lyrics are first mapped to sentiment score in R and the scores are classified to 8 kinds of moods: Anger, Anticipation, Disgust, Fear, Joy, Sadness, Surprise, and Trust. At the same time, the scores are binned into two categories, representing the positive sentiments and the negative sentiments respectively.
...
#### In the Talyor Swift example, the most frequent mood in the songs is positive. And we can see that Talyor Swift has expressed all kinds emotions in her songs. Joy, anticipation, and trust emerge as the top 3.
...
#### Now that we have figured out the overall sentiment scores, we should find out the top words that contribute to various emotions and positive/negative sentiment. The visualization is given shows that while the word bad is predominant in emotions such as anger, disgust, sadness, and fear, Surprise and trust are driven by the word good.
...
#### We can see that joy has the maximum share for the years 2010 and 2014. Overall, surprise, disgust, and anger are the emotions with the least score; however, in comparison to other years, 2017 has the maximum contribution for disgust. Coming to anticipation, 2010 and 2012 have a higher contribution in comparison to other years.

### (2). EDA for the whole lyrics dataset
#### In the second part of this project, we will furtherly study the trend and the sentiment for all the songs in Billboard just like the Talyor Swift example showed above. In the part of the project, we use some other visualizations graphed by seaborn to see the popularity of the songs and how it changed during the years or even months. These EDA plots could help us to find some interesting features in the dataset.
...
#### The graph above shows the density of relatively time x-axis and the lengths of weeks that songs stayed on board. It's easy to tell that the graph has two part of high density, which means in the middle time of these years. Around the year 2014, there are lot great songs been created. Also, around the year 2012, a lot of songs been created but did not stay on the board as long as the songs been created in 2014.
...
#### The graph above shows the number of favorites and retweets to the songs in ten years. As we can directly see, there is a huge difference between before the year 2017 and after the year 2017. After the year 2017, the number of favorites and retweets increased incredibly. That tells either the songs on the board have more influence after the year 2017 or people started to be crazy about using tweets.
...
#### The graph above shows the distribution of songs' popularity each year. As we can see, 2017 has the highest mean, which means most songs been created in 2017 are more popular than other years. However, for 2008, although the high point is really high this year, the mean is low. which means it has both good songs and bad songs this year.
...
#### The graph above shows the popularity of the songs in each month these years. From the graph, we also expected to see some trends through the changing of the color in the graph. And as the result we can tell from the graph, it did have a slight pattern of the increasing popularity from 2008 to around 2013. However, after that, the popularity of the songs each month became unpredictable.
...
#### Through the hypothesis tests we did above, we found out that tweets' attitude and lyrics' attitude of the songs have significant differences, and there is no significant linear relationship between them. However, that does not tell everything between them. Adding other possible related terms and use other forward tests may still get some good results. So, in the next part of the project, we gonna dig much deeper to discover the possibilities that they connect with each other, and how they may influence each other in any possible ways.

# Conclusion and Future Study
#### In the first part of this project, the team collects the data for Music Lyrics and Twitter and finishes the sentimental analysis on the lyrics and tweets. The EDA plots are used to partially answers the five sub-questions. In the second part of the project, our team will further study these successful popular songs and the words/sentiments in the lyrics. We will also try other methods to find out the patterns for the audiences' attitudes on the songs.
